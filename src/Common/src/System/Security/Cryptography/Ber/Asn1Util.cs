...
namespace System.Security.Cryptography
{
	static class Asn1Util
	{
		...
		public static byte[] DecodeBase64Array(byte[] srcArray)
		{
			...
			{
				throw new FormatException("srcArray Asn1InvalidEncodedDataLength");
			}
		...
		private static int DecodeBase64Char(byte c)
		{
			...
			if (num < 0)
			{
				throw new FormatException("c - Resources.Asn1IllegalCharacter");
			}
		...
		public static byte[] StringToBcd(string str)
		{
			...
			while (num3 < str.Length)
			{
				...
				if (!flag && ((c < 'A') || (c >= 'F')))
				{
					throw new FormatException("Asn1ValueParseException");
