            ...
            public override void ImportParameters(Gost3410Parameters parameters) ///!!!!!!
            {
                unsafe
                {
                    ...
                    fixed (byte* pgostBlob = &gostBlob[0])
                    {
                        ...
                        Interop.BCrypt.Emit(gostBlob, ref offset, System.Text.Encoding.UTF8.GetBytes(parameters.PublicKeyParamSet));
                        Interop.BCrypt.Emit(gostBlob, ref offset, System.Text.Encoding.UTF8.GetBytes(parameters.DigestParamSet));
                        Interop.BCrypt.Emit(gostBlob, ref offset, System.Text.Encoding.UTF8.GetBytes(parameters.EncryptionParamSet));
            ...
            private static void ExportParameters(ref Gost3410Parameters gostParams, byte[] gostBlob, bool includePrivateParameters)
            {
                unsafe
                {
                    ...
                    fixed (byte* pGostBlob = &gostBlob[0])
                    {
                        ...
                        int offset = sizeof(BCRYPT_GOSTKEY_BLOB);
                        var publicKeyParamSet = Interop.BCrypt.Consume(gostBlob, ref offset, pBcryptBlob->PublicKeyParamSet);
                        var digestParamSet = Interop.BCrypt.Consume(gostBlob, ref offset, pBcryptBlob->DigestParamSet);
                        var encryptionParamSet = Interop.BCrypt.Consume(gostBlob, ref offset, pBcryptBlob->EncryptionParamSet);

                        gostParams.PublicKeyParamSet = System.Text.Encoding.UTF8.GetString(digestParamSet, 0, digestParamSet.Length);
                        gostParams.DigestParamSet = System.Text.Encoding.UTF8.GetString(digestParamSet, 0, digestParamSet.Length);
                        gostParams.EncryptionParamSet = System.Text.Encoding.UTF8.GetString(encryptionParamSet, 0, encryptionParamSet.Length);
